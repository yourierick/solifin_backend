...
class TransactionFeeApiController extends Controller
{
    ...
    public function calculateWithdrawalFee(Request $request)
    {
        ...
        return response()->json([
            ...
            'data' => [
                'amount' => (float) $amount,
                'fee' => $fee,
                'total' => (float) $amount - $fee, // Pour un retrait, on soustrait les frais
                'currency' => $transactionFee->currency
            ]
        ]);
    }
    ...
    public function calculatePurchaseFee(Request $request)
    {
        ...
        $fee = $transactionFee->calculatePurchaseFee((float) $amount);
    ...
    /**
     * Calcule les frais d'achat de pack pour un montant et une méthode de paiement donnés.
     *
     * @param  \Illuminate\Http\Request  $request
     * @return \Illuminate\Http\Response
     */
    public function calculatePackPurchaseFee(Request $request)
    {
        $paymentMethod = $request->input('payment_method');
        $amount = $request->input('amount');
        ...
        switch ($paymentMethod) {
            ...
            case 'wallet':
                ...
                break;
            default:
                $feePercentage = 0.035; // 3.5% par défaut
        }
